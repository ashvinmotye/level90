...
assert.doesNotMatch(app,/saveStoicWeekReflection[\s\S]{0,1200}xpForQuest/,"Stoic reflections must not award XP");
assert.match(css,/\.stoic-week-cell\.current/);
assert.match(css,/\.stoic-week-cell\.future/);
assert.match(css,/\.stoic-save-state/);
assert.match(app,/function saveStoicWeekReflection\(/);
assert.match(app,/setStoicSaveState\("unsaved"\)/);
assert.doesNotMatch(app,/function scheduleStoicWeekSave\(/);
assert.match(app,/id="saveStoicWeekButton"[\s\S]*?>Save</);
...
assert.match(serviceWorker,/level90-v62/);
